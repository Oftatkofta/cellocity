...
Here we see an advantage in using PIV because processing times are orders of magnitude lower for the PIV data. It is possible to reduce the processing time of optical flow data by tweaking the ``r_step`` parameter of the ``._get_all_angles()`` metod of ``FiveSigmaAnalysis``. The default value is 1, which means that the comparrison is growing outward by 1 pixel per step, but a value of 2 would halve the number comparrisons calculated with little expected effect on the final result.
